...
$editLogistics = new Admin();
//fetch the vehicle data which id is in the URL
$editLogistics->fetchLogistics(intval($_GET['id']));
//update the URL requested Vehicle ID
$result = $editLogistics->editLogistics($_POST, intval($_GET['id']));
?>
            ...
            <div class="row g-3">
                <div class="col-md-6 mt-4">
                    ...
                    <textarea class="form-control" id="freight_details" name="freight_details" rows="3" required
                        placeholder="e.g. Transportation of textiles from Karachi to Lahore"><?php echo $editLogistics->freight_details ?? ""?></textarea>
                </div>
                <div class="col-md-6 mt-4">
                    ...
                    <textarea class="form-control" id="logistics_details" name="logistics_details" rows="3" required
                        placeholder="e.g. Goods will be transported via road. Estimated delivery time: 2 days."><?php echo $editLogistics->logistics_details ?? ""?></textarea>
                </div>
            </div>
            <div class="row g-3">
                <div class="col-md-6 mt-4">
                    ...
                    <input type="number" class="form-control" id="price" name="price" step="0.01" min="0" required
                        placeholder="e.g. 1500.00" value="<?php echo $editLogistics->price;?>">
                </div>
                <div class="col-md-6 mt-4">
                    <label for="price" class="form-label">Booking Date (till)</label>
                    <input type="date" class="form-control" id="booking_date" name="booking_date"
                        value="<?php echo $editLogistics->booking_date;?>" required>
                </div>
            </div>
            <div class="row g-3">
                <div class="col-md-6 mt-4">
                    ...
                    <input type="date" class="form-control" id="shipment_date" name="shipment_date"
                        value="<?php echo $editLogistics->shipment_date;?>" required>
                </div>
            </div>
            <div class="row g-3 mt-3">
                <div class="col-md-12">
                    <button type="submit" name="edit_logistics" class="btn btn-outline-primary"><i
                            class="fas fa-car"></i>&nbsp;Update</button>
